...
class Program004 : public WindowListener
{
private:
	bool running;
	bool keyDown[256];
...
public:
	...
	void G_Run()
	{
		...
		File::ReadText("data/shaders/point_to_quad.vert", vs, vsc);

		char *gs;
		unsigned int gsc;
		File::ReadText("data/shaders/point_to_quad.geom", gs, gsc);
		
		char *fs;
		unsigned int fsc;
		//File::ReadText("data/shaders/simple_multitexture.frag", fs, fsc);
		File::ReadText("data/shaders/point_to_quad.frag", fs, fsc);
		...
		//fetch and set the maximum amount of vertices that can be created
		int max_amount_of_vertices; //1024
		glGetIntegerv(GL_MAX_GEOMETRY_OUTPUT_VERTICES_EXT, &max_amount_of_vertices);
		glProgramParameteriEXT(p, GL_GEOMETRY_VERTICES_OUT_EXT, max_amount_of_vertices);
		...
		while( running )
		{
			...
			*/
			...
			glBindVertexArray(vaoHandle);
			glDrawArrays(GL_POINTS, 0, 3 ); //ooh draw the input type not the output type pretty tricky
